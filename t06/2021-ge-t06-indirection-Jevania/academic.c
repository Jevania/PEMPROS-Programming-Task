...
void calculate_gpa(struct student_t *_student,
  ...
  for(n=0; n<_enrollment_size; n++){
    if(strcmp(_enrollments[n].student.id, _student->id)==0){
      total_credit = total_credit + _enrollments[n].course.credit ;
      }
      _student->gpa = _student->gpa + calculate_performance(_enrollments[n].grade, _enrollments[n].course.credit);
  }
  _student->gpa = (_student->gpa)/(total_credit);
}
...
void set_enrollment_grade(struct course_t _course, struct student_t _student,
  ...
  int n;
  for(n=0; n<_enrollment_size; n++){
    if(strcmp(_enrollments[n].course.code, _course.code)==0 && strcmp(_enrollments[n].student.id, _student.id)==0){
      _enrollments[n].grade = _grade;
    }
  }
}
